    ...
    return (
        <div className="UserMenu" ref={refOne}>
            ...
            {openMenu && <div className="user-menu">
                {user && user.userRole === UserRole.admin && <button onClick={() => setOpenMenu(false)}>reports</button>}
                {user && user.userRole === UserRole.admin && <button onClick={() => setOpenMenu(false)}>download file</button>}
                {user && user.userRole === UserRole.admin && <button onClick={() => setOpenMenu(false)}>add vacation</button>}
